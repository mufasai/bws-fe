...
export const getProjectID = async (user_id: number) => {
  try {
    const token = localStorage.getItem('authToken');
    if (!token) throw new Error('Authentication token not found');

    // Misalnya, kamu ingin mendapatkan Project ID dengan melakukan GET request ke API
    ...
    // Mengembalikan data ID proyek yang didapatkan dari server
    const projectData = await response.json();
    return projectData.id; // Misalnya ID proyek ada di dalam data dengan properti 'id'
  } catch (error) {
