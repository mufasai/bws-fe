...
// const BASE_URL = 'http://192.168.1.2:8080/api/v1';
...
export const getProjectID = async (user_id: number) => {
  try {
    const response = await fetch(`${BASE_URL}/project.json`);
...
export const UpdateProjectAPI = async (id: number, data: CreateProjectRequest) => {
  try {
    const response = await fetch(`${BASE_URL}/project`, {
      method: 'PUT',
      headers: {
