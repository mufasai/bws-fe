...
const ShowFile = () => {
  const initialFiles = [
    {
      name: "List data number and message",
      date: "21 - 12 - 2024",
      type: "Excel",
      status: "success",
    },
  ...
  const filteredFiles = createMemo(() => {
    return initialFiles.filter((file) => {
      const dateMatch =
        selectedDate() === "Date" || file.date === selectedDate();
      const statusMatch =
        selectedStatus() === "Status" || file.status === selectedStatus();
          ...
          <tbody>
            {filteredFiles().map((file) => (
              <tr class="border-t">
                ...
                <td class="px-4 py-2">
                  <span
                    class={`px-2 py-1 rounded ${getStatusStyle(file.status)}`}
                  >
                    {file.status}
                  </span>
                </td>
